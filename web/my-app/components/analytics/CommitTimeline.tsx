...
export default function CommitTimeline({ data, title = "Commit Activity" }: Props) {
    ...
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6">
      ...
      <div className="relative h-48">
        ...
        <div className="absolute left-0 top-0 bottom-6 w-10 flex flex-col justify-between text-right pr-2">
          <span className="text-xs font-mono text-zinc-400 dark:text-zinc-600">{maxCount}</span>
          <span className="text-xs font-mono text-zinc-400 dark:text-zinc-600">{Math.round(maxCount / 2)}</span>
          <span className="text-xs font-mono text-zinc-400 dark:text-zinc-600">0</span>
        </div>
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{ height: `${Math.max(height, item.count > 0 ? 2 : 0)}%` }}
                  />
                  {/* Tooltip */}
